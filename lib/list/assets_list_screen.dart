...
class AssetsListScreen extends StatefulWidget {
  ...
  @override
  // Make the state class public by removing the underscore prefix
  AssetsListScreenState createState() => AssetsListScreenState();
}

// Change from private to public class (removed underscore)
class AssetsListScreenState extends State<AssetsListScreen> {
  static final Logger logger = Logger('AssetsListScreen');
  ...
  void initState() {
    super.initState();
    logger.info('AssetsListScreen initialized');
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<List<Project>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            logger.severe('Error fetching projects: ${snapshot.error}');
            return Center(
              child: Column(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          logger.info('Add investment button pressed');
          Navigator.push(
            context,
  ...
  Widget buildProjectCard(BuildContext context, Project project) {
    return Card(
      ...
      child: InkWell(
        onTap: () {
          logger.info('Project card tapped: ${project.id}');
          Navigator.push(
            context,
            MaterialPageRoute(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  IconButton(
                    onPressed: () => toggleArchiveStatus(project),
                    icon: Icon(
                  ...
                  // Delete button
                  IconButton(
                    onPressed: () => confirmDeleteProject(project),
                    icon: const Icon(
                      Icons.delete,
                      color: AssetFlowColors.error,
                    ),
                  ...
                  TextButton.icon(
                    onPressed: () {
                      logger.info('View details button pressed: ${project.id}');
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => AssetDetailScreen(projectId: project.id),
  ...
  void toggleArchiveStatus(Project project) {
    logger.info('Toggle archive status for project: ${project.id}');
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                if (project.isArchived) {
                  await databaseService.unarchiveProject(project.id);
                  if (mounted) {
                  ...
                  await databaseService.archiveProject(project.id);
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                ...
                logger.severe('Error toggling archive status: $e');
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
  ...
  void confirmDeleteProject(Project project) {
    logger.info('Delete project confirmation requested: ${project.id}');
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
  ...
  Future<void> deleteProject(String projectId) async {
    ...
    try {
      ...
      logger.severe('Error deleting project: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  void confirmSignOut() {
    logger.info('Sign out confirmation requested');
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
